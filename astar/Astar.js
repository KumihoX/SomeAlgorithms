    ...
    if (!isGenerated) {
        ...
        cells.forEach(function (element) {
            element.onclick = function() {
                ...
                else if (wallBuild && element.classList.contains("walkable")) {
                    element.className = "unwalkable";
                }
                else if (wallBuild && element.classList.contains("unwalkable")) {
...
function GenerateMaze() {
    if (!isGenerated) {
        alert('Для начала создайте пустой лабиринт!')
    }
    let size = document.getElementById('size').value;
...
function findPath() {
                ...
                if (!(closeMas.includes(newPoint)) && (newPoint.element.classList.contains('walkable'))){
                    if (!(openMas.includes(newPoint))){
                        ...
                        if (!newPoint.element.classList.contains('finishik')) {
                            var timerId_1 = setTimeout(paintingChild, count * delay, newPoint);
                            count += 1;
                        }
                        ...
                        if (!newPoint.element.classList.contains('finishik')) {
                            var timerId_2 = setTimeout(paintingParent, count * delay, newPoint);
                            count+=1;
                        }
    ...
    while (currentPoint.parent.parent !== null){
        ...
        var timerId_3 = setTimeout(paintingPath, count * delay, document.getElementById(currentPoint.parent.x + " " + currentPoint.parent.y));
        count += 1;
